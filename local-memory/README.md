...
搜索返回完整正文和元数据，可直接审查并用于修改；近期完整的 search/list/get/history 记录可以复用。仅在记录缺失、不完整或可能过时时按 ID 调用 `memory_get`，不逐条回读搜索命中。跨仓库读取需显式传 `scope: "all"`，仍受当前用户隔离，不授予修改权限。短 ID 在请求可见范围内有多个匹配时会报错，需提供更多字符；找不到记录也会报错，默认隐藏过期记录。

历史只允许本仓库，可读取过期记录，但不能通过工具读取已删除记录的历史；它保留正文变化，不是完整 metadata 审计。update 已回读持久化记录，直接检查返回值；记录不完整或可能过时时才需再次 get。get 确认存储内容，事实正确性需核对当前证据；需要检查召回质量时，可分别用一个标识符问题和一个自然语言问题回查。普通写入无需额外搜索，逐路分数也不证明精确标识符命中或事实正确。
...
`package.json` 里声明的是 `mem0ai@^3.2.0`，锁文件固定安装 3.2.0；跨大版本升级前先看上游 migration guide，尤其注意存储格式与 `search`/`getAll` 的参数约定，以及 [DESIGN.md 的「上游 mem0 的已知问题」](DESIGN.md#上游-mem0-的已知问题)那张表里还成不成立。
